...
exports.syncDailyArrivals = exports.testConnection = exports.stripeWebhook = exports.createDepositIntent = exports.analyzePassport = exports.syncReservationFromCloudbeds = void 0;
...
const ai = new genai_1.GoogleGenAI({ apiKey: GOOGLE_API_KEY });
// ----------------------------------------------------------------------
// HELPER: DATABASE LOGGING
// ----------------------------------------------------------------------
async function logEvent(reservationId, message, meta = {}) {
    try {
...
async function updateGuestStatus(reservationId, updates) {
    try {
...
async function callCloudbedsAPI(endpoint, params = {}, retryCount = 0) {
    ...
    try {
        ...
        const response = await axios_1.default.get(endpoint, { params: finalParams, headers, validateStatus: () => true });
...
// HELPER: DATA MAPPING
// ----------------------------------------------------------------------
function mapCloudbedsToApp(cloudbedsData) {
    return {
...
// ----------------------------------------------------------------------
exports.syncReservationFromCloudbeds = functions.https.onCall(async (data, context) => {
        ...
        if (targetId) {
            ...
            if (guestSearch.success && guestSearch.data && guestSearch.data.length > 0) {
                ...
                if (resSearch.success && resSearch.data && resSearch.data.length > 0) {
                    const relevant = resSearch.data.sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime())[0];
...
// ----------------------------------------------------------------------
exports.analyzePassport = functions.https.onCall(async (data, context) => {
    const payload = (data && data.data) ? data.data : data;
...
// ----------------------------------------------------------------------
exports.createDepositIntent = functions.https.onCall(async (data, context) => {
    const payload = (data && data.data) ? data.data : data;
...
exports.syncDailyArrivals = functions.https.onCall(async (data, context) => {
    try {
        ...
        for (const date of datesToCheck) {
            ...
            if (res.success && res.data) {
                for (const booking of res.data) {
                    if (processedIds.has(booking.reservationID))
                        continue; // Avoid duplicates across dates
