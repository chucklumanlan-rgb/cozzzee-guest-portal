...
// ----------------------------------------------------------------------
export const syncReservationFromCloudbeds = functions.https.onCall(async (dataOrRequest: any, context) => {
    // Handle both direct data (v1) and request.data (v2) structure
    const data = (dataOrRequest && dataOrRequest.data) ? dataOrRequest.data : dataOrRequest;
    const { reservationId, firstName, lastName, checkInDate } = data || {};
    
    const targetId = reservationId ? reservationId.trim() : null;
    ...
    try {
        ...
        if (!cloudbedsData && firstName && lastName) {
            ...
            if (guestSearch.success && guestSearch.data && guestSearch.data.length > 0) {
                 // Simplification: Take the first match that has a future reservation
                 const guestId = guestSearch.data[0].guestID;
                 // Get bookings for this guest
                 // This part requires chained calls in a real app. 
                 // For now, we abort complex search to keep it stable.
            }
        }
...
// ----------------------------------------------------------------------
export const createDepositIntent = functions.https.onCall(async (dataOrRequest: any, context) => {
    // Handle both direct data (v1) and request.data (v2) structure
    const data = (dataOrRequest && dataOrRequest.data) ? dataOrRequest.data : dataOrRequest;
...
export const syncDailyArrivals = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (res.success && res.data) {
            // In a real app, map all and batch write to Firestore
            // For now, we just return count
